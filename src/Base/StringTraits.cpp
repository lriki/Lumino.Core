...
int StringTraits::LastIndexOf(const TChar* str1, int str1Len, const TChar* str2, int str2Len, int startIndex, int count, CaseSensitivity cs)
{
	...
	if (LN_CHECKEQ_ARG(startIndex < 0)) return -1;			// startIndex は 0 以上でなければならない。
	if (LN_CHECKEQ_ARG(startIndex >= str1Len)) return -1;	// startIndex は str1 の長さを超えてはならない。
	...
	if (LN_CHECKEQ_ARG(end > pos)) return -1;						// 末尾と先頭が逆転してないこと。
...
{
	if (LN_CHECKEQ_ARG(begin == nullptr)) return;
	if (LN_CHECKEQ_ARG(length < 0)) return;
	if (LN_CHECKEQ_ARG(outBegin == nullptr)) return;
	if (LN_CHECKEQ_ARG(outLength == nullptr)) return;
