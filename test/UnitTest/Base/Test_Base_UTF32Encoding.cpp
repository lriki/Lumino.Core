...
TEST_F(Test_Base_UTF32Encoding, Basic)
{
	// UTF-32 → UTF-16
	{
		...
		RefPtr<RefBuffer> result1(Encoding::Convert(buf1, 4, decoder, encoder, &info));

		byte_t* utf16buf = (byte_t*)result1->GetPointer();
	...
	// UTF-16 → UTF-32
	{
		...
		EncodingConversionResult info;
		RefPtr<RefBuffer> result1(Encoding::Convert(uttf16buf, 2, decoder, encoder, &info));

		byte_t* utf32buf = (byte_t*)result1->GetPointer();
