...
{
#if 0
	StreamReader reader(LOCALFILE("TestData/Text_SJIS_CRLF.txt"), Text::Encoding::GetEncoding(Text::EncodingType_SJIS));
	String str, line;
	int count = 0;
	...
	// <Test> バッファリングバッファの境界で CRLF 改行文字が途切れていてもデコードできる。
	{
		...
		StreamReader reader(&mem, Text::Encoding::GetEncoding(Text::EncodingType_SJIS));
